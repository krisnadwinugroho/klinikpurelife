...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($data = mysqli_fetch_array($result)) {
        $response['data'] .= '<tr>
                                ...
                                <td>
                                    <a href="update_pasien.php?id=' . $data['id'] . '" class="btn btn-warning" role="button">Update</a>
    ...
    $sql_total = "SELECT COUNT(*) AS total FROM pasien";
    if ($search) {
        $sql_total .= " WHERE nama_pasien LIKE '%$search%'";
    }
    ...
    $response['data'] = '<div class="alert alert-info">Tidak ada data pasien yang ditemukan.</div>';
    $response['pagination'] = '';
}
